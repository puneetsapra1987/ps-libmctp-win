...
PKTBUF_STORAGE_ALIGN_DECL static uint8_t test_tx_storage[MCTP_PKTBUF_SIZE(MCTP_BTU)] PKTBUF_STORAGE_ALIGN;
			       ...
			       struct mctp *test_endpoint, void *callback_ctx)
{
	...
	test_binding->tx_storage = test_tx_storage;
...
static void send_transport_control_message(void)
{
	...
	const struct msg_payload send_control_message_payload = {
		.hdr = {
			.dest = eid_1,
			.src = eid_2,
